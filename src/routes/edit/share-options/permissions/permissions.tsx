...
export const Permissions = ({ shareOptionsState, setShareOptionsState }: any) => {
	...
	// selected permission in the dropdown is 'read only' and the chart switches from a non public
	// chart to a public chart.
	...
	const removePermission = (rowIndex: number) => {
		setShareOptionsState({
			...shareOptionsState,
			chartPermissions: shareOptionsState.chartPermissions
				.filter((_: any, i: number) => i !== rowIndex)
		});
	};

	const changePermission = (rowIndex: number, permission: string) => {
		const newPermissions = [...shareOptionsState.chartPermissions];
		newPermissions[rowIndex].permission = permission;
		setShareOptionsState({
			...shareOptionsState,
			chartPermissions: newPermissions
		});
	};

	const addPermission = (submit: any) => {
		if (shareOptionsState.chartPermissions
		...
		setShareOptionsState({
			...shareOptionsState,
			chartPermissions: [
				...shareOptionsState.chartPermissions,
	...
	return (
		<>
			<Checkbox
				...
				defaultChecked={shareOptionsState.isChartPublic}
				labelText={
					<>
						...
						<TooltipDefinition
							tooltipText=
								'Making a chart public removes all users with Read only priviledges'>
							public
						</TooltipDefinition>
					</>
				}
					...
					<div className={permissionDropdown}>
						<Controller
							...
							as={
								<Dropdown
									invalid={false}
									invalidText='Can not add read only permission if chart is public'
									disabled={shareOptionsState.isChartPublic}
			...
			<StructuredListWrapper className={structuredList}>
				...
				<StructuredListBody>
					{shareOptionsState.chartPermissions.map((chartPermission: any, rowIndex: number) => {
						if (chartPermission.permission === 'Read only' && shareOptionsState.isChartPublic) {
							return null;
						}
						return (
							<StructuredListRow key={chartPermission.email}>
								<StructuredListCell
									className={listCell}
									tabIndex={0}
									aria-label={`Permissions for ${chartPermission.email}`}>
									{chartPermission.email}
								</StructuredListCell>
								<StructuredListCell className={listCell}>
									<Dropdown
										...
										itemToString={(item: any) => (item ? item.text : '')}
										initialSelectedItem={{ text: chartPermission.permission }}
										label={`Modify permission for ${chartPermission.email}`} />
								</StructuredListCell>
								<StructuredListCell className={deleteCell}>
									<Button
										kind='ghost'
										onClick={() => removePermission(rowIndex)}
										aria-label={`Delete permission for ${chartPermission.email}`}>
										<Delete20 />
									</Button>
								</StructuredListCell>
							</StructuredListRow>
